...
import {
  ...
  type GetAuthorInput,
  type GetWorkInput,
} from './tools/openLibraryTools.js';
...
export class MCPServer {
  private openLibraryTools: OpenLibraryTools;
  ...
  constructor(config: MCPServerConfig) {
    this.config = config;
    this.openLibraryTools = new OpenLibraryTools();
  }
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...SEARCH_BOOKS_DEF,
          inputSchema: OpenLibraryTools.getSearchBooksSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_BOOK_BY_ISBN_DEF,
          inputSchema: OpenLibraryTools.getGetBookByIsbnSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_AUTHOR_DEF,
          inputSchema: OpenLibraryTools.getGetAuthorSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_WORK_DEF,
          inputSchema: OpenLibraryTools.getGetWorkSchema().inputSchema as Tool['inputSchema'],
        },
      ];
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'search_books':
            result = await this.openLibraryTools.executeSearchBooks(
              safeArgs as unknown as SearchBooksInput
            );
            break;
          case 'get_book_by_isbn':
            result = await this.openLibraryTools.executeGetBookByIsbn(
              safeArgs as unknown as GetBookByIsbnInput
            );
            break;
          case 'get_author':
            result = await this.openLibraryTools.executeGetAuthor(
              safeArgs as unknown as GetAuthorInput
            );
            break;
          case 'get_work':
            result = await this.openLibraryTools.executeGetWork(
              safeArgs as unknown as GetWorkInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for Open Library API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3505;
