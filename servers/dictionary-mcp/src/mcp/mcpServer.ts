...
export class MCPServer {
  ...
  constructor(config: MCPServerConfig) {
    this.config = config;
    this.dictionaryTools = new DictionaryTools();
  }
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...DEFINE_WORD_DEF,
          inputSchema: DictionaryTools.getDefineWordSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_SYNONYMS_DEF,
          inputSchema: DictionaryTools.getGetSynonymsSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_PHONETICS_DEF,
          inputSchema: DictionaryTools.getGetPhoneticsSchema().inputSchema as Tool['inputSchema'],
        },
      ];
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'define_word':
            result = await this.dictionaryTools.executeDefineWord(
              safeArgs as unknown as DefineWordInput
            );
            break;
          case 'get_synonyms':
            result = await this.dictionaryTools.executeGetSynonyms(
              safeArgs as unknown as GetSynonymsInput
            );
            break;
          case 'get_phonetics':
            result = await this.dictionaryTools.executeGetPhonetics(
              safeArgs as unknown as GetPhoneticsInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for Dictionary API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3507;
