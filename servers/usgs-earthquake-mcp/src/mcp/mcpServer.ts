...
export class MCPServer {
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...QUERY_EARTHQUAKES_DEF,
          inputSchema: UsgsTools.getQueryEarthquakesSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_RECENT_SIGNIFICANT_DEF,
          inputSchema: UsgsTools.getGetRecentSignificantSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_EVENT_DEF,
          inputSchema: UsgsTools.getGetEventSchema().inputSchema as Tool['inputSchema'],
        },
      ];
      ...
      try {
        ...
        switch (name) {
          case 'query_earthquakes':
            result = await this.usgsTools.executeQueryEarthquakes(
              safeArgs as unknown as QueryEarthquakesInput
            );
            break;
          case 'get_recent_significant':
            result = await this.usgsTools.executeGetRecentSignificant(
              safeArgs as unknown as GetRecentSignificantInput
            );
            break;
          case 'get_event':
            result = await this.usgsTools.executeGetEvent(
              safeArgs as unknown as GetEventInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for USGS API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3509;
