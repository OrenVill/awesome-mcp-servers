...
export class MCPServer {
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...SEARCH_MDN_DEF,
          inputSchema: MdnTools.getSearchMdnSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_DOC_DEF,
          inputSchema: MdnTools.getGetDocSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_BROWSER_COMPAT_DEF,
          inputSchema: MdnTools.getGetBrowserCompatSchema().inputSchema as Tool['inputSchema'],
        },
      ];
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'search_mdn':
            result = await this.mdnTools.executeSearchMdn(
              safeArgs as unknown as SearchMdnInput
            );
            break;
          case 'get_doc':
            result = await this.mdnTools.executeGetDoc(
              safeArgs as unknown as GetDocInput
            );
            break;
          case 'get_browser_compat':
            result = await this.mdnTools.executeGetBrowserCompat(
              safeArgs as unknown as GetBrowserCompatInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for MDN API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3512;
