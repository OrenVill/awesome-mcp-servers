...
export function getConfig(): ServiceConfig {
  ...
  return {
    ...
    api: {
      baseUrl: process.env.NOMINATIM_BASE_URL ?? fileConfig.api.baseUrl,
      timeoutMs: parseInt(
        process.env.NOMINATIM_TIMEOUT_MS ?? String(fileConfig.api.timeoutMs),
        10
      ),
      userAgent: process.env.NOMINATIM_USER_AGENT ?? fileConfig.api.userAgent,
    },
  };
}
