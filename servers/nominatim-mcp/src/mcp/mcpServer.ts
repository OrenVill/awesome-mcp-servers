...
export class MCPServer {
  private nominatimTools: NominatimTools;
  ...
  constructor(config: MCPServerConfig) {
    this.config = config;
    this.nominatimTools = new NominatimTools();
  }
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...GEOCODE_DEF,
          inputSchema: NominatimTools.getGeocodeSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...REVERSE_GEOCODE_DEF,
          inputSchema: NominatimTools.getReverseGeocodeSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...LOOKUP_DEF,
          inputSchema: NominatimTools.getLookupSchema().inputSchema as Tool['inputSchema'],
        },
      ];
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'geocode':
            result = await this.nominatimTools.executeGeocode(
              safeArgs as unknown as GeocodeInput
            );
            break;
          case 'reverse_geocode':
            result = await this.nominatimTools.executeReverseGeocode(
              safeArgs as unknown as ReverseGeocodeInput
            );
            break;
          case 'lookup':
            result = await this.nominatimTools.executeLookup(
              safeArgs as unknown as LookupInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for Nominatim API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3506;
