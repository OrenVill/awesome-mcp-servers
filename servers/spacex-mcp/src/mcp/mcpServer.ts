...
export class MCPServer {
  private spacexTools: SpacexTools;
  ...
  constructor(config: MCPServerConfig) {
    this.config = config;
    this.spacexTools = new SpacexTools();
  }
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...GET_LATEST_LAUNCH_DEF,
          inputSchema: SpacexTools.getGetLatestLaunchSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_NEXT_LAUNCH_DEF,
          inputSchema: SpacexTools.getGetNextLaunchSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...LIST_LAUNCHES_DEF,
          inputSchema: SpacexTools.getListLaunchesSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_LAUNCH_DEF,
          inputSchema: SpacexTools.getGetLaunchSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_ROCKET_DEF,
          inputSchema: SpacexTools.getGetRocketSchema().inputSchema as Tool['inputSchema'],
        },
      ];
      ...
      try {
        ...
        switch (name) {
          case 'get_latest_launch':
            result = await this.spacexTools.executeGetLatestLaunch(
              safeArgs as unknown as GetLatestLaunchInput
            );
            break;
          case 'get_next_launch':
            result = await this.spacexTools.executeGetNextLaunch(
              safeArgs as unknown as GetNextLaunchInput
            );
            break;
          case 'list_launches':
            result = await this.spacexTools.executeListLaunches(
              safeArgs as unknown as ListLaunchesInput
            );
            break;
          case 'get_launch':
            result = await this.spacexTools.executeGetLaunch(
              safeArgs as unknown as GetLaunchInput
            );
            break;
          case 'get_rocket':
            result = await this.spacexTools.executeGetRocket(
              safeArgs as unknown as GetRocketInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for SpaceX API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3510;
