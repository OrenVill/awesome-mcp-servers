...
export function getConfig(): ServiceConfig {
  ...
  return {
    ...
    api: {
      firebaseBaseUrl:
        process.env.HN_FIREBASE_BASE_URL ?? fileConfig.api.firebaseBaseUrl,
      algoliaBaseUrl:
        process.env.HN_ALGOLIA_BASE_URL ?? fileConfig.api.algoliaBaseUrl,
      timeoutMs: parseInt(
        process.env.HN_TIMEOUT_MS ?? String(fileConfig.api.timeoutMs),
        10
      ),
    },
  };
