...
export class MCPServer {
  private triviaTools: TriviaTools;
  ...
  constructor(config: MCPServerConfig) {
    this.config = config;
    this.triviaTools = new TriviaTools();
  }
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...GET_QUESTIONS_DEF,
          inputSchema: TriviaTools.getGetQuestionsSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...LIST_CATEGORIES_DEF,
          inputSchema: TriviaTools.getListCategoriesSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_CATEGORY_COUNT_DEF,
          inputSchema: TriviaTools.getGetCategoryCountSchema().inputSchema as Tool['inputSchema'],
        },
      ];
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'get_questions':
            result = await this.triviaTools.executeGetQuestions(
              safeArgs as unknown as GetQuestionsInput
            );
            break;
          case 'list_categories':
            result = await this.triviaTools.executeListCategories(
              safeArgs as unknown as ListCategoriesInput
            );
            break;
          case 'get_category_count':
            result = await this.triviaTools.executeGetCategoryCount(
              safeArgs as unknown as GetCategoryCountInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for Open Trivia DB (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3514;
