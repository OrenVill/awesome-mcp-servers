...
export class MCPServer {
  private crossrefTools: CrossrefTools;
  ...
  constructor(config: MCPServerConfig) {
    this.config = config;
    this.crossrefTools = new CrossrefTools();
  }
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...GET_WORK_DEF,
          inputSchema: CrossrefTools.getGetWorkSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...SEARCH_WORKS_DEF,
          inputSchema: CrossrefTools.getSearchWorksSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...SEARCH_JOURNALS_DEF,
          inputSchema: CrossrefTools.getSearchJournalsSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_JOURNAL_DEF,
          inputSchema: CrossrefTools.getGetJournalSchema().inputSchema as Tool['inputSchema'],
        },
      ];
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'get_work':
            result = await this.crossrefTools.executeGetWork(
              safeArgs as unknown as GetWorkInput
            );
            break;
          case 'search_works':
            result = await this.crossrefTools.executeSearchWorks(
              safeArgs as unknown as SearchWorksInput
            );
            break;
          case 'search_journals':
            result = await this.crossrefTools.executeSearchJournals(
              safeArgs as unknown as SearchJournalsInput
            );
            break;
          case 'get_journal':
            result = await this.crossrefTools.executeGetJournal(
              safeArgs as unknown as GetJournalInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for Crossref API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3515;
