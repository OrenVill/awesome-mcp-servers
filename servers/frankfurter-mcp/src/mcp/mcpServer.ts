...
} from './tools/frankfurterTools.js';
...
export class MCPServer {
  private frankfurterTools: FrankfurterTools;
  ...
  constructor(config: MCPServerConfig) {
    this.config = config;
    this.frankfurterTools = new FrankfurterTools();
  }
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...GET_LATEST_RATES_DEF,
          inputSchema: FrankfurterTools.getGetLatestRatesSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...CONVERT_CURRENCY_DEF,
          inputSchema: FrankfurterTools.getConvertCurrencySchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_HISTORICAL_RATES_DEF,
          inputSchema: FrankfurterTools.getGetHistoricalRatesSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_TIME_SERIES_DEF,
          inputSchema: FrankfurterTools.getGetTimeSeriesSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...LIST_CURRENCIES_DEF,
          inputSchema: FrankfurterTools.getListCurrenciesSchema().inputSchema as Tool['inputSchema'],
        },
      ];
      ...
      try {
        ...
        switch (name) {
          case 'get_latest_rates':
            result = await this.frankfurterTools.executeGetLatestRates(
              safeArgs as unknown as GetLatestRatesInput
            );
            break;
          case 'convert_currency':
            result = await this.frankfurterTools.executeConvertCurrency(
              safeArgs as unknown as ConvertCurrencyInput
            );
            break;
          case 'get_historical_rates':
            result = await this.frankfurterTools.executeGetHistoricalRates(
              safeArgs as unknown as GetHistoricalRatesInput
            );
            break;
          case 'get_time_series':
            result = await this.frankfurterTools.executeGetTimeSeries(
              safeArgs as unknown as GetTimeSeriesInput
            );
            break;
          case 'list_currencies':
            result = await this.frankfurterTools.executeListCurrencies(
              safeArgs as unknown as ListCurrenciesInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for Frankfurter API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3508;
