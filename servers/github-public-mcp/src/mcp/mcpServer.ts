...
export class MCPServer {
  ...
  private createServerWithHandlers(): Server {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const tools: Tool[] = [
        {
          ...GET_REPO_DEF,
          inputSchema: GithubTools.getGetRepoSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...GET_USER_DEF,
          inputSchema: GithubTools.getGetUserSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...LIST_REPO_ISSUES_DEF,
          inputSchema: GithubTools.getListRepoIssuesSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...LIST_REPO_RELEASES_DEF,
          inputSchema: GithubTools.getListRepoReleasesSchema().inputSchema as Tool['inputSchema'],
        },
        {
          ...SEARCH_REPOS_DEF,
          inputSchema: GithubTools.getSearchReposSchema().inputSchema as Tool['inputSchema'],
        },
      ];
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          case 'get_repo':
            result = await this.githubTools.executeGetRepo(
              safeArgs as unknown as GetRepoInput
            );
            break;
          case 'get_user':
            result = await this.githubTools.executeGetUser(
              safeArgs as unknown as GetUserInput
            );
            break;
          case 'list_repo_issues':
            result = await this.githubTools.executeListRepoIssues(
              safeArgs as unknown as ListRepoIssuesInput
            );
            break;
          case 'list_repo_releases':
            result = await this.githubTools.executeListRepoReleases(
              safeArgs as unknown as ListRepoReleasesInput
            );
            break;
          case 'search_repos':
            result = await this.githubTools.executeSearchRepos(
              safeArgs as unknown as SearchReposInput
            );
            break;
          default:
  ...
  async initialize(): Promise<void> {
    // No async init needed for GitHub API (stateless HTTP)
  }
  ...
  async startHttp(): Promise<void> {
    ...
    const port = this.config.httpPort ?? 3511;
