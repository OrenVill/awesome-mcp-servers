...
export function getConfig(): ServiceConfig {
  ...
  return {
    ...
    api: {
      restBaseUrl:
        process.env.WIKIPEDIA_REST_URL ?? fileConfig.api.restBaseUrl,
      mediaWikiBaseUrl:
        process.env.WIKIPEDIA_MEDIAWIKI_URL ?? fileConfig.api.mediaWikiBaseUrl,
      timeoutMs: parseInt(
        process.env.WIKIPEDIA_TIMEOUT_MS ?? String(fileConfig.api.timeoutMs),
        10
      ),
    },
  };
